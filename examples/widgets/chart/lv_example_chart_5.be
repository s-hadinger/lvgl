...
def slider_x_event_cb(slider, e)
    var v = slider.get_value()
    chart.set_zoom_x(v)
end

def slider_y_event_cb(slider, e)
    var v = slider.get_value()
    chart.set_zoom_y(v)
end
...
chart.align(lv.ALIGN_CENTER, -30, -30)
chart.set_range(lv.CHART_AXIS_PRIMARY_Y, -1000, 1000)

# Do not display points on the data
chart.set_style_size(0, lv.PART_INDICATOR)

ser = chart.add_series(lv.palette_main(lv.PALETTE_RED), lv.CHART_AXIS_PRIMARY_Y)

pcnt = size(ecg_sample_int)
chart.set_point_count(pcnt)
ecg_sample = lv.coord_arr(ser.y_points, pcnt)
#chart.set_ext_y_array(ser, ecg_sample)
for i:0..pcnt-1
    ecg_sample[i] = ecg_sample_int[i]
end

slider = lv.slider(lv.scr_act())
slider.set_range(lv.IMG_ZOOM_NONE, lv.IMG_ZOOM_NONE * 10)
slider.add_event_cb(slider_x_event_cb, lv.EVENT_VALUE_CHANGED, nil)
slider.set_size(200,10)
slider.align_to(chart, lv.ALIGN_OUT_BOTTOM_MID, 0, 20)

slider = lv.slider(lv.scr_act())
slider.set_range(lv.IMG_ZOOM_NONE, lv.IMG_ZOOM_NONE * 10)
slider.add_event_cb(slider_y_event_cb, lv.EVENT_VALUE_CHANGED, nil)
slider.set_size(10, 150)
slider.align_to(chart, lv.ALIGN_OUT_RIGHT_MID, 20, 0)
